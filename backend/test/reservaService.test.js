...
} from "../src/modules/reservas/reserva.service.js";
import * as reservaServiceFacade from "../src/services/reservaService.js";
...
test("rejeita criacao quando o cliente nao existe ou o e-mail nao foi validado", async (t) => {
  mockTransaction(t);
  const cliente = { id: 10, email: "cliente@teste.com", status: CLIENTE_STATUS.ATIVO };
  let buscasCliente = 0;
  t.mock.method(Cliente, "findByPk", async () => {
    buscasCliente += 1;
    return buscasCliente === 1 ? null : cliente;
  });
  ...
  await assert.rejects(
    () => criarReserva({
      ...
      modalidadeId: 30,
      horarioId: 40,
      emailVerificado: { email: "cliente@teste.com" },
    }),
    (erro) => erro instanceof ErroDaAplicacao && erro.status === 409,
  );
...
test("rejeita quadra inexistente ou inativa e horario inexistente", async (t) => {
  mockTransaction(t);
  const cliente = { id: 10, email: "cliente@teste.com", status: CLIENTE_STATUS.ATIVO };
  t.mock.method(Cliente, "findByPk", async () => cliente);
  let buscasQuadra = 0;
  t.mock.method(Quadra, "findByPk", async () => {
    buscasQuadra += 1;
    if (buscasQuadra === 1) return null;
    if (buscasQuadra === 2) return { id: 20, status: "inativa", modalidades: [] };
    return { id: 20, status: QUADRA_STATUS.ATIVA, modalidades: [{ id: 30 }] };
  });
  ...
  await assert.rejects(
    () => criarReserva({
      ...
      modalidadeId: 30,
      horarioId: 40,
      emailVerificado: { email: cliente.email },
    }),
    (erro) => erro instanceof ErroDaAplicacao && erro.status === 409,
  );
  ...
  t.mock.method(Modalidade, "findByPk", async () => ({ id: 30, status: MODALIDADE_STATUS.ATIVA }));
...
test("faz rollback da transacao quando a criacao da reserva falha", async (t) => {
  ...
  const cliente = { id: 10, email: "cliente@teste.com", status: CLIENTE_STATUS.ATIVO };
  const quadra = { id: 20, status: QUADRA_STATUS.ATIVA, valorHora: 100, modalidades: [{ id: 30 }] };
  const horario = {
    id: 40,
    quadraId: quadra.id,
    data: "2099-01-06",
  ...
  t.mock.method(Horario, "findByPk", async () => horario);
  t.mock.method(Reserva, "findOne", async () => null);
  t.mock.method(Reserva, "create", async () => {
    throw new Error("falha simulada na persistencia");
  });
  ...
  await assert.rejects(
    ...
    /falha simulada na persistencia/,
  );
  assert.equal(rollbackExecutado, true);
});

test("nega cancelamento quando o status atual nao permite a transicao", async (t) => {
  mockTransaction(t);
  const reserva = {
    id: 51,
    status: RESERVA_STATUS.FINALIZADA,
  ...
  await assert.rejects(
    () => alterarStatusDaReserva({
      id: reserva.id,
      statusEsperados: [RESERVA_STATUS.AGUARDANDO_PAGAMENTO, RESERVA_STATUS.CONFIRMADA],
      novoStatus: RESERVA_STATUS.CANCELADA,
      adminId: 1,
      enderecoIp: "127.0.0.1",
    }),
...
test("mantem a fachada reservaService compativel com pagamentos e consumidores existentes", () => {
  assert.equal(reservaServiceFacade.criarReserva, criarReserva);
  assert.equal(reservaServiceFacade.alterarStatusDaReserva, alterarStatusDaReserva);
  assert.equal(reservaServiceFacade.verificarHorarioDisponivel, verificarHorarioDisponivel);
});
